...
// top of the previous one (by ~20 deploys the demo held 1,216 invoices and every
// customer was far over its credit limit, which — with live-balance credit
// checks — parked every new order ON_HOLD and skewed all AR/dashboard metrics).
//
...
// tables (order_lines, POS line items, AP/match rows, …) not listed explicitly;
// it cannot reach reference tables because those are never referenced BY the
// transactional tables. Errors are logged, not fatal (mirrors dedupeBranches).

...
// AI_LM (Load Management & Compliance) integration surface. AI_LM is a
// standalone sidecar service: it pulls vehicles, drivers, a calendar date's
// confirmed orders (with per-unit weight + PIM geometry resolved per line) and
// the product catalog, then writes approved delivery routes — including the
// 3D packing manifest that powers the yard "Pack Trucks" instructions — back
// into the dispatch board. All endpoints are X-Integration-Key gated.
...
type integrationOrderLine struct {
	...
	WeightLbs float64 `json:"weight_lbs"` // per-unit
}
...
type integrationRouteStop struct {
...
type integrationDeliveryRoute struct {
	VehicleID     string                 `json:"vehicle_id"`
	DriverID      string                 `json:"driver_id,omitempty"`
	ScheduledDate string                 `json:"scheduled_date"` // YYYY-MM-DD
	Stops         []integrationRouteStop `json:"stops"`
	LoadManifest  json.RawMessage        `json:"load_manifest,omitempty"`
}

// --- fleet ----------------------------------------------------------------

// ListVehiclesIntegration returns the active fleet.
func (h *Handler) ListVehiclesIntegration(w http.ResponseWriter, r *http.Request) {
	rows, err := h.db.Pool.Query(r.Context(), `
		SELECT id, name, vehicle_type, COALESCE(license_plate, ''), capacity_weight_lbs,
		       COALESCE(make, ''), COALESCE(model, ''), COALESCE(year, 0)
		FROM vehicles WHERE deleted_at IS NULL ORDER BY name`)
	if err != nil {
		slog.Error("integration: list vehicles", "error", err)
		writeError(w, http.StatusInternalServerError, "failed to query vehicles")
		return
	}
	defer rows.Close()

	out := []integrationVehicle{}
	for rows.Next() {
		var v integrationVehicle
		if err := rows.Scan(&v.ID, &v.Name, &v.VehicleType, &v.LicensePlate, &v.CapacityWeightLbs, &v.Make, &v.Model, &v.Year); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to read vehicle data")
			return
		}
		out = append(out, v)
	}
...
// ListDriversIntegration returns drivers (ACTIVE first ordering is the caller's concern).
func (h *Handler) ListDriversIntegration(w http.ResponseWriter, r *http.Request) {
	rows, err := h.db.Pool.Query(r.Context(), `
		SELECT id, name, COALESCE(status, 'ACTIVE')
		FROM drivers WHERE deleted_at IS NULL ORDER BY name`)
	if err != nil {
		slog.Error("integration: list drivers", "error", err)
		writeError(w, http.StatusInternalServerError, "failed to query drivers")
		return
	}
	defer rows.Close()

	out := []integrationDriver{}
	for rows.Next() {
		var d integrationDriver
		if err := rows.Scan(&d.ID, &d.Name, &d.Status); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to read driver data")
			return
		}
		out = append(out, d)
	}
...
// ListOrdersIntegration returns orders scheduled for delivery on ?date=
// (YYYY-MM-DD, required) filtered by ?status= (default CONFIRMED), with line
// items carrying per-unit weight and the order's delivery geopoint.
func (h *Handler) ListOrdersIntegration(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.db.Pool.Query(r.Context(), `
		...
		WHERE o.scheduled_delivery_date = $1 AND o.status = $2
		ORDER BY o.created_at`, date, status)
	if err != nil {
		slog.Error("integration: list orders", "error", err)
	...
	ids := []string{}
	for rows.Next() {
		var o integrationOrder
		if err := rows.Scan(&o.ID, &o.Status, &o.CustomerName, &o.Address, &o.Latitude, &o.Longitude, &o.ScheduledDate); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to read order data")
			return
		}
		o.Lines = []integrationOrderLine{}
	...
	if len(ids) > 0 {
		lineRows, err := h.db.Pool.Query(r.Context(), `
			SELECT ol.order_id, ol.product_id, p.sku, ol.quantity, COALESCE(p.weight_lbs, 0)
			FROM order_lines ol
			JOIN products p ON p.id = ol.product_id
			WHERE ol.order_id = ANY($1::uuid[])
			ORDER BY ol.created_at`, ids)
		if err != nil {
			slog.Error("integration: list order lines", "error", err)
			writeError(w, http.StatusInternalServerError, "failed to query order lines")
			return
		}
		defer lineRows.Close()

		byID := make(map[string]*integrationOrder, len(orders))
		for i := range orders {
			byID[orders[i].ID] = &orders[i]
		}
		for lineRows.Next() {
			var orderID string
			var l integrationOrderLine
			if err := lineRows.Scan(&orderID, &l.ProductID, &l.SKU, &l.Quantity, &l.WeightLbs); err != nil {
				writeError(w, http.StatusInternalServerError, "failed to read order line data")
				return
			}
...
// CreateDeliveryRouteIntegration writes an AI_LM-approved route into the
// dispatch board: one delivery_routes row (status SCHEDULED) plus one
// deliveries row per stop. Idempotent on (vehicle_id, scheduled_date) — an
// existing non-dispatched route for that pair is replaced, so re-approving a
// plan updates the board instead of duplicating it.
func (h *Handler) CreateDeliveryRouteIntegration(w http.ResponseWriter, r *http.Request) {
	var req integrationDeliveryRoute
	...
	// Replace any not-yet-dispatched route for this vehicle+date.
	_, err = tx.Exec(ctx, `
		DELETE FROM deliveries WHERE route_id IN (
			SELECT id FROM delivery_routes
			WHERE vehicle_id = $1 AND scheduled_date = $2 AND status IN ('DRAFT', 'SCHEDULED')
		)`, vehicleID, req.ScheduledDate)
	if err == nil {
		_, err = tx.Exec(ctx, `
	...
	if err != nil {
		slog.Error("integration: replace existing route", "error", err)
	...
	var routeID string
	err = tx.QueryRow(ctx, `
		INSERT INTO delivery_routes (vehicle_id, driver_id, scheduled_date, status, notes, load_manifest)
		VALUES ($1, $2, $3, 'SCHEDULED', 'Optimized by AI Load Management', $4)
		RETURNING id`, vehicleID, driverID, req.ScheduledDate, req.LoadManifest).Scan(&routeID)
	if err != nil {
		slog.Error("integration: create route", "error", err)
	...
	for _, stop := range req.Stops {
		...
		if err != nil {
			writeError(w, http.StatusBadRequest, fmt.Sprintf("invalid order_id %q", stop.OrderID))
			return
		}
		_, err = tx.Exec(ctx, `
			INSERT INTO deliveries (route_id, order_id, stop_sequence, status, latitude, longitude)
			VALUES ($1, $2, $3, 'PENDING', $4, $5)`,
			routeID, orderID, stop.Sequence, stop.Lat, stop.Lng)
		if err != nil {
			slog.Error("integration: create delivery stop", "error", err, "order_id", stop.OrderID)
			writeError(w, http.StatusInternalServerError, "failed to create delivery stop")
			return
		}
	}

	if err := tx.Commit(ctx); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to commit route")
		return
	}

	writeJSON(w, http.StatusCreated, map[string]string{"route_id": routeID, "status": "SCHEDULED"})
}

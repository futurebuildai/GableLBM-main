...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, roleGuard ...func(http.Handler) http.Handler) {
	...
	mux.HandleFunc("GET /api/reports/daily-till", guard(h.HandleDailyTill))
	mux.HandleFunc("GET /api/reports/sales-summary", guard(h.HandleSalesSummary))
	mux.HandleFunc("GET /api/reports/ar-aging", guard(h.HandleARAgingReport))
	mux.HandleFunc("GET /api/reports/customer-statement/{id}", guard(h.HandleCustomerStatement))
}
...
func (h *Handler) RegisterBuilderRoutes(mux *http.ServeMux, roleGuard ...func(http.Handler) http.Handler) {
	...
	mux.HandleFunc("POST /api/reporting/builder/preview", guard(h.HandleBuilderPreview))
	mux.HandleFunc("POST /api/reporting/builder/export", guard(h.HandleBuilderExport))
	mux.HandleFunc("POST /api/reporting/save", guard(h.HandleSaveReport))
	mux.HandleFunc("GET /api/reporting/saved", guard(h.HandleListSavedReports))
	mux.HandleFunc("GET /api/reporting/saved/{id}", guard(h.HandleGetSavedReport))
	mux.HandleFunc("PUT /api/reporting/saved/{id}", guard(h.HandleUpdateSavedReport))
	mux.HandleFunc("DELETE /api/reporting/saved/{id}", guard(h.HandleDeleteSavedReport))
}

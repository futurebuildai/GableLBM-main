...
var entityBaseQuery = map[string]string{
	...
	"customers":       "FROM customers cu",
	"products":        "FROM products pr LEFT JOIN vendors v ON pr.vendor_id = v.id",
	"payments":        "FROM payments pay",
	"purchase_orders": "FROM purchase_orders po LEFT JOIN vendors v ON po.vendor_id = v.id",
}

...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, roleGuard ...func(http.Handler) http.Handler) {
	...
	mux.HandleFunc("GET /locations/{id}", guard(h.GetLocation))
	mux.HandleFunc("PUT /locations/{id}", adminGuard(h.UpdateLocation))
	mux.HandleFunc("DELETE /locations/{id}", adminGuard(h.DeleteLocation))

	// Branch CRUD.
	mux.HandleFunc("GET /branches", guard(h.ListBranches))
	mux.HandleFunc("POST /branches", adminGuard(h.CreateBranch))
	mux.HandleFunc("GET /branches/{id}", guard(h.GetBranch))
	mux.HandleFunc("PUT /branches/{id}", adminGuard(h.UpdateBranch))
	mux.HandleFunc("DELETE /branches/{id}", adminGuard(h.DeleteBranch))
	mux.HandleFunc("GET /branches/{id}/tree", guard(h.GetBranchTree))

	// User-branch grants.
	if h.userRepo != nil {
		mux.HandleFunc("GET /me/branches", guard(h.ListMyBranches))
		mux.HandleFunc("GET /users", adminGuard(h.ListKnownUsers))
		mux.HandleFunc("GET /users/{sub}/branches", adminGuard(h.ListUserBranches))
		mux.HandleFunc("POST /users/{sub}/branches", adminGuard(h.GrantUserBranch))
		mux.HandleFunc("DELETE /users/{sub}/branches/{branch_id}", adminGuard(h.RevokeUserBranch))
		mux.HandleFunc("PUT /users/{sub}/home-branch", adminGuard(h.SetHomeBranch))
		mux.HandleFunc("GET /branches/{id}/users", adminGuard(h.ListBranchUsers))
	}
}

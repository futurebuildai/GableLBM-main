...
func (h *ExposureHandler) HandleReportExposure(w http.ResponseWriter, r *http.Request) {
	...
	if summary {
		writeJSON(w, http.StatusOK, map[string]any{
			...
			"delta_vs_prior_week_dollars": rollup.DeltaVsPriorWeekDollars,
		})
		return
	}
...
// single-valued FB Brain "role" claim if present, else the first entry of
// "roles". Empty string when no claims at all.

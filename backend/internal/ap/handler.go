...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, roleGuard ...func(http.Handler) http.Handler) {
	...
	// Vendor Invoices
	mux.HandleFunc("POST /api/ap/invoices", guard(h.CreateVendorInvoice))
	mux.HandleFunc("GET /api/ap/invoices", guard(h.ListVendorInvoices))
	mux.HandleFunc("GET /api/ap/invoices/{id}", guard(h.GetVendorInvoice))
	mux.HandleFunc("POST /api/ap/invoices/{id}/approve", guard(h.ApproveInvoice))

	// AP Payments
	mux.HandleFunc("POST /api/ap/payments", guard(h.PayVendor))
	mux.HandleFunc("GET /api/ap/payments", guard(h.ListPayments))

	// Aging Report
	mux.HandleFunc("GET /api/ap/aging", guard(h.GetAgingSummary))
}
